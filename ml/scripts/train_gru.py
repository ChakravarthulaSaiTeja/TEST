#!/usr/bin/env python3
"""
GRU Model Training Script for Stock Price Prediction
...
class GRUModel(nn.Module):
    """GRU model for stock price prediction - EXACT notebook architecture"""

    def __init__(self, input_size, output_size=1):
        super(GRUModel, self).__init__()

        # Layer 1: GRU(128, return_sequences=True) + Dropout(0.3) - EXACT notebook
        self.gru1 = nn.GRU(
        ...
        # Layer 2: GRU(64) + Dropout(0.2) - EXACT notebook
        self.gru2 = nn.GRU(
            input_size=128,
            hidden_size=64,
            num_layers=1,
    ...
    def forward(self, x):
        # Forward propagate through first GRU
        out, _ = self.gru1(x)
        out = self.dropout1(out)

        # Forward propagate through second GRU
        out, _ = self.gru2(out)
...
def prepare_data(symbol, sequence_length=120, test_split=0.2):
    """Prepare data for GRU training - matching notebook approach"""
...
def train_model(
    ...
    """Train the GRU model with early stopping and learning rate reduction"""
...
def save_model(model, scaler, symbol, model_dir=None):
    ...
    model_path = model_dir / f"gru_{symbol.lower()}.pth"
    torch.save(model.state_dict(), model_path)
    logger.info(f"GRU model saved to {model_path}")

    # Save scaler (.pkl format)
    scaler_path = model_dir / f"gru_{symbol.lower()}_scaler.pkl"
    import pickle
...
def main():
    parser = argparse.ArgumentParser(description="Train GRU model for stock prediction")
    ...
    parser.add_argument(
        "--hidden_size", type=int, default=128, help="Hidden size for GRU"
    )
    parser.add_argument(
    ...
    try:
        logger.info(f"Starting GRU training for {args.symbol}")
        ...
        logger.info(f"GRU training completed successfully for {args.symbol}")
